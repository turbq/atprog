...
def set_debug_exec(address, data):
    log.info("RI4ProgramInterface: Debug exec at 0x%x, is %d bytes long" & (
        address, len(data)))
...
def set_sw_bp(address, instruction, flags):
    ...
    read_transfer("pgm", absolute_start_addr, data_buffer, buffer_size)
    ...
    if single_page_access:
        ...
        waitForFlashReady()
        write_transfer("Pgm", absolute_start_addr, data_buffer, buffer_size)
    return org_inst
